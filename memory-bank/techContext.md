...
- **Stdout contract.** Our stdout is reserved for the final JSON result. Codex's own chatter is captured silently by default and only mirrored to stderr with `--stream-thinking`; pipes consuming our stdout get exactly one valid JSON document. `--quiet` suppresses live mirroring even when requested. Bounded stdout/stderr tails are captured for failure reporting.

## Codex behaviors (empirical)
- **`--full-auto` is deprecated** in codex 0.128.0. Using it emits `warning: \`--full-auto\` is deprecated; use \`--sandbox workspace-write\` instead.` on stderr. The wrapper uses the explicit canonical form: `--sandbox <mode>` and no longer accepts a `full-auto` wrapper permission.
...
- **`--sandbox` values** (codex 0.128.0): `read-only`, `workspace-write`, `danger-full-access`. The wrapper's `--permissions` maps directly to these names.
...
- **`--model` validation is server-side.** Unsupported values return HTTP 400 with `"<model> is not supported when using Codex with a ChatGPT account"`. No way to enumerate supported models from the CLI; the wrapper just lists common known names in `--help`, including GPT-5.6 tiers `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna`.
- **`--reasoning-effort` validation is server-side.** The wrapper passes it as `-c model_reasoning_effort="<level>"` and does not enumerate values locally. Probed 2026-07-09 on codex-cli 0.144.1: terra accepts `none|minimal|low|medium|high|xhigh`; sol additionally accepts `max` and `ultra`; unsupported effort/model pairs fail from codex with a diagnostic mentioning `model_reasoning_effort` / reasoning effort.
- **Web search.** The wrapper exposes no `--search` flag. Empirically, `codex exec` can access web search when the prompt explicitly asks for current web research, so callers should express that need in `--prompt` / `--prompt-file`.
...
- **Action verb hygiene.** Codex sometimes uses synonyms like `"modified"`, `"updated"`, or `"removed"` instead of the requested `"edited"` / `"deleted"`. The wrapper coerces unknown verbs to `"referenced"` and warns. Referenced entries are omitted unless `--track-references`.
- **Stdout chatter.** Codex's stdout under `exec` is verbose by design (reasoning traces, tool calls, partial outputs). The wrapper does not stream it by default. `--stream-thinking` mirrors it live to wrapper stderr for human observation. Stdout/stderr are always tailed for error reporting.
- **Preflight and run errors.** Runtime calls `codex --version` before `codex exec` so a missing/not-runnable CLI returns structured JSON instead of a raw spawn failure. Non-zero `codex exec` exits include diagnostic tails and hints for auth (`codex login`), quota/rate limits, unsupported models, rejected reasoning effort, and sandbox/filesystem denials. `--retries` replays only non-zero transient model-capacity / 429 / rate-limit / temporarily-unavailable / overloaded and sandbox-wrapper prep failures; durable exclusions win first.
...
## Tooling patterns
- **CLI shape:** single task mode plus an installer-dispatch mode. Task mode — required: a prompt via inline `--prompt` or from a file `--prompt-file` (UTF-8, `.trim()`-applied, mutually exclusive, empty-after-trim rejected). Optional: `--cwd` (relative or absolute; must exist), `--out` (write JSON to this path in addition to stdout), `--debug` (preserve scratch on success), `--quiet` (suppress live mirroring), `--stream-thinking` (mirror codex stdout/stderr to wrapper stderr), `--track-references` (keep referenced files), `--no-install-check` (skip the skill-registration warning; env equivalent `CODEX_TASK_SKIP_INSTALL_CHECK=1`), `--retries <N>` (non-negative integer; default `0`; sequential retries for non-zero transient model-capacity/sandbox-wrapper prep failures only), `--model <NAME>` (default `gpt-5.5`; verbatim pass-through), `--reasoning-effort <LEVEL>` (optional; verbatim pass-through to `-c model_reasoning_effort="<level>"`), `--permissions <MODE>` (default `read-only`; one of `read-only|workspace-write|danger-full-access`), `--profile <NAME>` (optional pass-through). `full-auto` is rejected as deprecated. `-h`/`--help` prints usage to stdout and exits 0 (POSIX — pipeable). Usage-due-to-error (missing required arg, unknown arg, --retries missing/negative/non-integer value, --reasoning-effort missing value, --cwd missing, unreadable/empty prompt file, invalid permissions value) prints to stderr and exits 2. Installer mode — any of `--install` / `--uninstall` / `--list-targets` in a flag position (the detection scan skips values of value-taking task flags, so `--prompt "--install"` is a task) forwards the argv (minus `--install` / `--no-install-check`) to the sibling `install.mjs` via `spawnSync(process.execPath, ...)` with inherited stdio and exits with the child's status; runs before parseArgs so installer flags like `--target=` never hit task-arg validation. Missing install.mjs, `spawnSync` `error`, and `signal` death each print a plain stderr diagnostic and exit 1 (no JSON — installer mode is not a task run).
- **Skill-registration warning:** before a task run, probe `skillProbePaths()` — `<home>/{.claude,.config/opencode,.cline,.cursor,.agents}/skills/codex-task/SKILL.md` (kept manually in sync with `TARGETS` in install.mjs; the installed `~/.codex-task/codex-task.mjs` must not hard-import install.mjs since older installs shipped the tool alone). If none exists: one-line warning to stderr + same message in the JSON `warnings` array. Never blocks, never changes exit code; the probe is try/catch-guarded because `os.homedir()` throws on Windows when HOME/USERPROFILE are empty — a probe failure degrades to a `warnings` entry. Tests set `CODEX_TASK_SKIP_INSTALL_CHECK=1` in their base env and redirect `HOME`/`USERPROFILE` at temp dirs for the dedicated check tests (which cover: warning fires on bare home, silent when registered, silent via flag, silent via env var, lookalike `--prompt "--install"` stays a task, and homedir-failure resilience).
- **Output contract:** JSON on stdout via `emit()`. Base shape: `ok`, `taskResult`, `summary`, `details`, `files`, `workdir`, `sessionDir`, `model`, `permissions`, `reasoningEffort`, `warnings`, `durationMs`; `attempts` is added only when `--retries > 0`; `error` is added on failure. Exit code 0 iff `taskResult` is `completed`; 2 for argparse errors; 1 for runtime/task failures.
- **Error model:** all failure paths route through `emit(..., 1|2, args.out)` with a populated `error` and `sessionDir` preserved when applicable. No partial JSON, no half-written stdout. The `--out` file gets the same content as stdout on every exit path. Clean exit-0 task results are never retried, so Windows sandbox blocked runs that return `taskResult:"blocked"` require manual re-dispatch rather than `--retries`.
...
- **Result lenience:** strip a leading ` ```json `/` ``` ` fence and trailing ``` before parsing. Strict JSON.parse first. On failure, scan for the first balanced `{...}` block (depth + JSON-string-quote aware) and parse that — emits a warning when this fallback fires. Validate `taskResult`, coerce unknown action verbs (`"modified"` etc.) to `"referenced"` with warnings, and omit referenced entries unless `--track-references`.
...
- **Permissions → sandbox mapping** (table in `PERMISSIONS` constant): `read-only` → `--sandbox read-only`; `workspace-write` → `--sandbox workspace-write`; `danger-full-access` → `--sandbox danger-full-access`. Approval is always `never` regardless.
...
- `.gitignore` — standard Node / OS-noise ignores plus `tmp/` for manual harness artifacts. The wrapper itself does not write into the project directory.
...
- `tests/cli-smoke.test.mjs` — Node test smoke coverage for CLI help, installer target listing, runtime→installer flag forwarding, and the skill-registration warning (present when no harness registered; absent when registered or disabled).
- `scripts/permission-matrix.mjs` — manual Codex sandbox matrix; accepts `--target DIR`, defaults to `../kva`, and is intentionally not wired into `npm test`.
