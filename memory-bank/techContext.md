...
## Tooling patterns
- **CLI shape:** single mode, no subcommand. Required: a prompt via inline `--prompt` or from a file `--prompt-file` (UTF-8, `.trim()`-applied, mutually exclusive, empty-after-trim rejected). Optional: `--cwd` (relative or absolute; must exist), `--out` (write JSON to this path in addition to stdout), `--debug` (preserve scratch on success), `--quiet` (suppress live mirroring), `--stream-thinking` (mirror codex stdout/stderr to wrapper stderr), `--track-references` (keep referenced files), `--model <NAME>` (default `gpt-5.5`; verbatim pass-through), `--permissions <MODE>` (default `read-only`; one of `read-only|workspace-write|danger-full-access`), `--profile <NAME>` (optional pass-through). `full-auto` is rejected as deprecated. `-h`/`--help` prints usage to stdout and exits 0 (POSIX — pipeable). Usage-due-to-error (missing required arg, unknown arg, --cwd missing, unreadable/empty prompt file, invalid permissions value) prints to stderr and exits 2.
- **Output contract:** JSON on stdout via `emit()`. Always the same shape: `ok`, `taskResult`, `summary`, `details`, `files`, `workdir`, `sessionDir`, `model`, `permissions`, `warnings`, `durationMs`; `error` added on failure. Exit code 0 iff `taskResult` is `completed`; 2 for argparse errors; 1 for runtime/task failures.
