...
- **Output contract:** JSON on stdout via `emit()`. Always the same shape: `ok`, `taskResult`, `summary`, `details`, `files`, `workdir`, `sessionDir`, `model`, `permissions`, `reasoningEffort`, `warnings`, `durationMs`; `error` added on failure. Exit code 0 iff `taskResult` is `completed`; 2 for argparse errors; 1 for runtime/task failures.
