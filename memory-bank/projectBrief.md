...
- Must preflight `codex --version` before a run and return structured JSON when the Codex CLI is missing or not runnable. Non-zero `codex exec` exits include a captured diagnostic tail plus hints for common auth, quota, model, and sandbox failures.
