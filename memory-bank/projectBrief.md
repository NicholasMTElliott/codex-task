...
- Codex pass-throughs exposed as wrapper flags: `--model` (default `gpt-5.5`), `--permissions` (default `read-only`; values map to codex's `--sandbox`: `read-only` / `workspace-write` / `full-auto` alias / `danger-full-access`), `--profile` (optional pass-through to codex's `--profile`). The wrapper always passes `--ephemeral` (each delegation is one-shot, not part of a persisted session) and captures the agent's final message via codex's `--output-last-message`. Approval defaults to `never` automatically in `codex exec` (and `--ask-for-approval` is not exposed on the exec subcommand).
- Codex flags NOT exposed: `--search` (top-level codex only; not available on `codex exec` in 0.128.0 — related feature flags are deprecated). `--ask-for-approval` (also top-level only; exec defaults to `never`).
